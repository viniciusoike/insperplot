...
# Wrapped in try() to prevent vignette build failure if fonts unavailable
suppressMessages(suppressWarnings({
  try(insperplot::import_insper_fonts(verbose = FALSE), silent = TRUE)
}))
```
...
ggplot(paulista, aes(x = date, y = value)) +
  geom_line(color = get_insper_colors("teals1"), linewidth = 0.8) +
  theme_insper() +
  labs(
...
ggplot(spo_metro |> filter(name_station == "Paulista"),
       aes(x = date, y = value)) +
  geom_line(color = get_insper_colors("teals1"), linewidth = 1.2) +
