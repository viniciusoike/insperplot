...
ggplot(metro |> filter(name_station == "Paulista"),
       aes(x = date, y = value)) +
  geom_line(color = insper_col("teals1"), linewidth = 1.2) +
  theme_insper() +
  labs(
...
Use `insper_col()` to access Insper's brand colors:
...
# Get multiple colors
insper_col("reds1", "teals1", "oranges1")
...
show_insper_palette("reds_seq")
show_insper_palette("teals_seq")
```
...
get_insper_colors("qualitative_main")
```
...
```{r barplot}
# Compare average 2020 ridership by station
metro |>
  filter(year == 2020) |>
...
inner_join(paulista, pinheiros, by = "date") |>
  insper_scatterplot(
    x = paulista,
    y = pinheiros,
    color = as.numeric(format(date, "%Y")),
    add_smooth = TRUE,
