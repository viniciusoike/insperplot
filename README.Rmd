...
# Visualization chunk temporarily disabled - to be fixed
library(treemapify)
library(insperplot)
library(ggplot2)
```
...
**insperplot now intelligently detects whether you're specifying a static color or mapping a variable!**

```{r smart-detection, eval=FALSE}
...
You can extract hex color codes directly from palettes using `get_palette_colors()`:

```{r palette-colors, eval=FALSE}
